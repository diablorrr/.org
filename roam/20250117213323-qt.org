...
** tmp精简版
Qt 信号槽机制底层依赖元对象编译器（moc）。
moc 扫描含Q_OBJECT宏的类，生成moc_XXX.cpp，其中包含元对象信息。
QObject::connect时，通过qt_static_metacall的QMetaObject::IndexOfMethod分支获取信号与槽的元索引，将 “发送者指针、信号索引、接收者指针、槽函数索引” 等连接信息存入 QObject 内部连接列表，同时确定连接类型。
信号触发，发送者的 moc 文件中信号函数调用QMetaObject::activate()，它按连接类型处理内部连接列表里的 “接收者 - 槽” 对。例如直接连接，立即在当前线程调用接收者qt_static_metacall；队列连接则将QMetaCallEvent放入接收者线程事件队列。
qt_static_metacall走QMetaObject::InvokeMetaMethod分支，依槽函数元索引调用槽函数，并处理参数传递。
