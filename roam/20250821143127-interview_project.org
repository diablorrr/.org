...
** 问答
*** YES 介绍下你的项目
- 项目介绍           :: 这个一个基于 reactor模型 实现的 高并发TCP服务器框架（在这个TCP服务器框架上封装了 EchoServer），one thread one loop，具有 事件驱动、可扩展性高、线程安全 等特点；使用 =epoll + 非阻塞IO= 达到百万级连接能力
- 整体架构与线程模型 :: 在TCP层，（主线程）主事件循环绑定Accept，负责监听、接受新连接；（其他线程）从事件循环线程池中的从事件循环绑定 Connection，负责连接的IO；在应用层，（其他线程）有可选的工作线程池负责业务处理（计算密集型任务）；上下层之间通信方式 上层向下层注册回调函数，下层根据触发事件类型的同步调用不同回调函数
...
2. TcpMgr 收到来自服务端的 搜索用户回包（ID_SEARCH_USER_RSP），解析JSON，将用户搜索结果交给 SearchList
