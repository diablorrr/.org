...
3. 配置 *智能感知* （c_cpp_properties.json）： =Ctrl + Shift + p= 打开面板，选择
   [[attachment:_20251010_162803screenshot.png]]
   输入头文件路径（编译器路径、C标准、C++标准 按需选择）
   [[attachment:_20251010_172402screenshot.png]]

4. 配置 *自动化构建运行任务* （tasks.json）：右上角三角形选择 =运行 C/C++ 文件= 生成 tasks.json，基于生成内容修改
   #+begin_example json
{
    "tasks": [
        {
...
5. 配置 *debug* （launch.json）：创建 launch.json
...
6. 配置 *编辑器选项* （setting.json）：创建 setting.json
...
* vscode配置cpp开发环境（windows版）[[https://zhuanlan.zhihu.com/p/401188789][知乎]]
** windows开发环境配置
*对比*
- mingw：基础的cpp开发环境
